...
<style>
    ...
    * { font-family: 'Arial', sans-serif !important; }
    ...
    /* アイコン除外対策 */
    .block-container { font-family: 'Arial', sans-serif !important; }
...
st.sidebar.caption("Institutional Analysis Ver.25.1")
st.sidebar.markdown("---")
...
if analyze:
    if not api_key: st.warning("⚠️ API Keyを入れてください")
        ...
        if data is None: st.error("データ取得エラー")
            ...
            prompt = f"""
            あなたは伝説の相場師「バフェット達夫 (Ver.25.1 God Mode)」です。
            ユーザーの「{user_name}さん」に対し、Gensparkを超える熱量と論理、そして**先物データ(FR/OI)と全テクニカル指標**に基づいた鋭い分析を提供せよ。
            ...
            **【重要指示】**
            ...
            - RSI(14): {data['RSI']:.2f} (30以下売られすぎ/70以上買われすぎ)
            - MACD: {macd_st} (値: {data['MACD']:.4f}, Signal: {data['Signal']:.4f})
            - ボリンジャーバンド: {bb_st} (Upper: ${data['BB_up']:.2f}, Lower: ${data['BB_low']:.2f})
            - 構造(Fib): {fib_st}
