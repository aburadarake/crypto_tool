...
st.markdown("""
<style>
    /* フォント設定 (アイコン対策: body以下に適用) */
    .stApp, .stApp p, .stApp h1, .stApp h2, .stApp h3, .stApp li, .stApp td, .stApp th {
        font-family: 'Arial', sans-serif !important;
    }
    
    /* アイコン用のクラスは除外 (Streamlitの仕様に合わせる) */
    i, .material-icons, [data-testid="stSidebarNav"] {
...
st.sidebar.caption("Ver.28.0 Fixed Edition")
st.sidebar.markdown("---")

user_name = st.sidebar.text_input("お名前", value="ユーザー")
api_key = st.sidebar.text_input("Google API Key", type="password", help="APIキーを入力してください")
...
# ★Bybit API (修正: ヘッダー追加とエラーハンドリング強化)★
...
if analyze:
    if not api_key: st.warning("⚠️ API Keyを入れてください")
        ...
        if data is None: st.error("データ取得エラー")
            ...
            with m3: st.markdown(f'<div class="metric-card"><div style="color:#848e9c;font-size:12px;">Funding Rate (Bybit)</div><div style="color:{fr_col};font-size:24px;font-weight:bold;">{fr_disp}</div></div>', unsafe_allow_html=True)
            ...
            prompt = f"""
            あなたは伝説の相場師「バフェット達夫 (Ver.28.0 Fixed)」です。
            ...
            * **展開:** ...
            ...
            ### 🚨 シナリオC：緊急撤退 (確率 10%)
            * **撤退ライン:** ${sl_long:.2f} (L) / ${sl_short:.2f} (S)

            ## 💰 推奨エントリー指示書 (厳守)
            * **推奨アクション:** [成行エントリー / 指値待ち / 様子見]
            * **適正ロット数:** 約 **{rec_lot:.4f} {symbol_input}**
            * **損切り(SL):** **${sl_long:.2f} (L) / ${sl_short:.2f} (S)**
