...
import requests
import numpy as np
import ccxt  # 必須: requirements.txtに ccxt を入れてください
...
def check_password():
    ...
    st.markdown("""
    <style>
        body, h1, h2, h3, h4, h5, h6, p, span, div, label, button, input, textarea {
...
# ★最強データ取得関数 (Binance -> Bybit -> Coinglass(APIなし)の代替)★
def get_crypto_derivatives(symbol):
    ticker = f"{symbol.upper()}USDT"
    headers = {'User-Agent': 'Mozilla/5.0'}
    
    # 1. Binance Futures (最優先)
    try:
        # Binanceはccxt経由の方が安定する場合がある
        exchange = ccxt.binance()
        fr = float(exchange.fapiPublicGetPremiumIndex({'symbol': ticker})['lastFundingRate'])
        oi = float(exchange.fapiPublicGetOpenInterest({'symbol': ticker})['openInterest'])
        return fr, oi
    except: pass # 次へ
    ...
    try:
        ...
        return float(result['fundingRate']), float(result['openInterest'])
    except: pass

    # 3. それでもダメなら0を返す (AIには「データなし」と伝える)
    return 0.0, 0.0
...
if analyze:
    if not api_key: st.warning("⚠️ API Keyを入れてください")
        ...
        if data is None: st.error("データ取得エラー")
            ...
            if atr_val == 0: atr_val = cp * 0.02 # データ不足時は2%を仮置き
