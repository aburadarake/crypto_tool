...
def check_password():
    ...
    <style>
        * { font-family: 'Arial', sans-serif !important; }
        ...
        h1 { color: #F0B90B; letter-spacing: 2px; font-size: 2.5em; } /* 見出しは大きく */
        .stTextInput input { 
...
st.sidebar.caption("Institutional Analysis Ver.24.1")
st.sidebar.markdown("---")

user_name = st.sidebar.text_input("お名前", value="ユーザー")
# ▼▼▼ 修正: helpを削除して「key...」を消す ▼▼▼
...
if analyze:
    if not api_key: st.warning("⚠️ API Keyを入れてください")
        ...
        if data is None: st.error("データ取得エラー")
            ...
            # ★神プロンプト (機関投資家視点を追加)★
            prompt = f"""
            あなたは機関投資家の戦略を熟知した伝説の相場師「バフェット達夫 (Ver.24.1 Institutional)」です。
