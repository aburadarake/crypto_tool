...
def check_password():
    ...
    if st.session_state["auth_status"]: return True

    # CSS修正: アイコン文字化け対策 (最新版)
    st.markdown("""
    <style>
        /* 全体フォント設定 */
        .stApp { 
        ...
        /* アイコンフォントを守る (重要) */
        i, .material-icons, [class*="st-emotion-cache"], [data-testid="stSidebarNav"] span {
...
<style>
    /* フォント設定 (再定義) */
    .stApp, .stApp p, .stApp div, .stApp span, .stApp label, .stApp button, .stApp input {
        font-family: 'Arial', sans-serif !important;
    }
    /* アイコンを守る */
    [class^="st-"], [class*=" st-"], .material-icons {
        font-family: inherit !important;
    }
...
st.sidebar.caption("Ver.36.0 Fixed Edition")
st.sidebar.markdown("---")
...
def detect_patterns(df):
    ...
    if lower > body * 2 and upper < body * 0.5: patterns.append("ハンマー(反転上昇)")
    if upper > body * 2 and lower < body * 0.5: patterns.append("ピンバー(反転下落)")
    if row['Close'] > row['Open'] and prev['Close'] < prev['Open'] and row['Close'] > prev['Open']: patterns.append("包み足(強)")
...
if analyze:
    if not api_key: st.warning("⚠️ API Keyを入れてください")
        ...
        if data is None: st.error("データ取得エラー")
            ...
            prompt = f"""
            あなたは伝説の相場師「バフェット達夫 (Ver.36.0 Final)」です。
