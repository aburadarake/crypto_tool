...
def check_password():
    ...
    <style>
        ...
        /* アイコンはデフォルトフォントに戻す */
        .material-icons, [data-testid="stSidebarNav"] {
...
st.markdown("""
<style>
    :not(.material-icons) { font-family: 'Arial', sans-serif; }
    .stApp { background-color: #0b0e11; color: #e1e1e1; font-size: 16px; }
...
st.sidebar.caption("Ver.34.0 Final Edition")
st.sidebar.markdown("---")
...
def detect_patterns(df):
    ...
    # ハンマー/カラカサ (下ヒゲが実体の2倍以上)
    if lower_shadow > body_size * 2 and upper_shadow < body_size * 0.5:
        patterns.append("ハンマー(反転上昇示唆)")
        
    # 上ヒゲピンバー (上ヒゲが実体の2倍以上)
    if upper_shadow > body_size * 2 and lower_shadow < body_size * 0.5:
        patterns.append("上ヒゲピンバー(下落示唆)")
        
    # 包み足 (Bullish Engulfing)
    if row['Close'] > row['Open'] and prev['Close'] < prev['Open'] and row['Close'] > prev['Open'] and row['Open'] < prev['Close']:
        patterns.append("強気の包み足(上昇転換)")
        
    # 包み足 (Bearish Engulfing)
    if row['Close'] < row['Open'] and prev['Close'] > prev['Open'] and row['Close'] < prev['Open'] and row['Open'] > prev['Close']:
        patterns.append("弱気の包み足(下落転換)")
...
def plot_chart(df, symbol):
    ...
    # BB (Upper/Lower) - 範囲で表示
    fig.add_trace(go.Scatter(x=df.index, y=df['BB_up'], line=dict(color='rgba(255, 255, 255, 0.3)', width=1), name='BB Upper'))
    fig.add_trace(go.Scatter(x=df.index, y=df['BB_low'], line=dict(color='rgba(255, 255, 255, 0.3)', width=1), name='BB Lower', fill='tonexty'))
    
    # SMA 20 / 200
    fig.add_trace(go.Scatter(x=df.index, y=df['SMA_20'], line=dict(color='#F0B90B', width=1.5), name='SMA 20 (Short Trend)'))
    fig.add_trace(go.Scatter(x=df.index, y=df['SMA_200'], line=dict(color='#FFFFFF', width=2, dash='solid'), name='SMA 200 (Long Trend)'))
...
if analyze:
    if not api_key: st.warning("⚠️ API Keyを入れてください")
        ...
        if data is None: st.error("データ取得エラー")
            ...
            # BB判定
            bb_st = "レンジ内"
            if cp > data['BB_up']: bb_st = "バンドウォーク警戒(買われすぎ)"
            elif cp < data['BB_low']: bb_st = "売られすぎ(反発警戒)"
            ...
            # ★神プロンプト (テクニカル完全網羅・具体的数値入り)★
            prompt = f"""
            あなたは伝説の相場師「バフェット達夫 (Ver.34.0 God Mode)」です。
            ユーザー: {user_name}, モード: {risk_mode}
            
            **【重要指示】**
            1. 数字はプレーンテキストで出力せよ。装飾記号は使うな。
