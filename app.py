...
st.sidebar.caption("Institutional Analysis Ver.26.0")
st.sidebar.markdown("---")
...
def analyze_patterns(df):
    # 直近5期間の高値・安値
    last_5_highs = df['High'].tail(5).values
    last_5_lows = df['Low'].tail(5).values
    
    pattern = "特になし"
    
    # 高値切り下げ (三尊・下落トレンド示唆)
    if last_5_highs[0] < last_5_highs[1] > last_5_highs[2] > last_5_highs[3]:
        pattern = "高値切り下げ (下落圧力)"
    
    # 安値切り上げ (アセンディング・上昇示唆)
    if last_5_lows[0] > last_5_lows[1] < last_5_lows[2] < last_5_lows[3]:
        pattern = "安値切り上げ (上昇圧力)"
...
if analyze:
    if not api_key: st.warning("⚠️ API Keyを入れてください")
        ...
        if data is None: st.error("データ取得エラー")
            ...
            prompt = f"""
            あなたは伝説の相場師「バフェット達夫 (Ver.26.0 God Mode)」です。
            ...
            **【重要指示】**
            ...
            【📊 テクニカル & チャートパターン】
            - パターン認識: {data['Pattern']} (高値/安値の推移から判定)
            - トレンド: {trend} (SMA200)
