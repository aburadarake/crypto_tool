...
# --- ページ設定 ---
st.set_page_config(page_title="TATSUO INSTITUTIONAL", page_icon="🦁", layout="wide")
...
<style>
    ...
    div[data-testid="stMetricValue"] { font-size: 24px !important; color: #fff !important; }
...
# --- サイドバー設定 ---
st.sidebar.title("🦁 TATSUO INSTITUTIONAL")
st.sidebar.caption("Ver.23.0 On-Chain Analysis")
st.sidebar.markdown("---")
...
if analyze:
    if not api_key: st.warning("⚠️ API Keyを入れてください")
        ...
        if data is None: st.error("データ取得エラー")
            ...
            # ★神プロンプト (機関投資家視点を追加)★
            prompt = f"""
            あなたは機関投資家の戦略を熟知した伝説の相場師「バフェット達夫 (Ver.23.0 Institutional)」です。
            ...
            【🐋 機関投資家の動向 (最重要)】
            - **金利(Funding Rate):** {funding_rate*100:.4f}%
              (0.01%が基準。プラス乖離ならロング過多で暴落警戒。マイナスならショート燃料あり)
            - **建玉(Open Interest):** ${open_interest:,.0f}
              (価格上昇+OI増加=本物。価格上昇+OI減少=ショートカバー/ダマシ)
            - **需給(MFI):** {data['MFI']:.1f}
            
            【テクニカル構造】
            - トレンド: {trend} (SMA200) / 雲: {cloud}
            - 構造: {fib_st}
