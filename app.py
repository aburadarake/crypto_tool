...
def check_password():
    ...
    <style>
        /* ▼▼▼ アイコンに影響しないフォント指定 ▼▼▼ */
        .block-container, .stTextInput, .stButton {
...
<style>
    ...
    /* 本文サイズ固定 */
    p, li, span, div { font-size: 16px !important; line-height: 1.6 !important; }
    ...
    /* AIボックス調整 */
    .ai-box p, .ai-box li { font-size: 16px !important; color: #e1e1e1 !important; }
    .ai-box strong { font-weight: bold !important; color: #F0B90B !important; font-size: 16px !important; }
...
if analyze:
    if not api_key: st.warning("⚠️ API Keyを入れてください")
        ...
        if data is None: st.error("データ取得エラー")
            ...
            bb_st = "バンドウォーク" if cp > data['BB_up'] else "レンジ"
            ...
            # ★フォーマット修正: 数字に余計な装飾をつけないよう指示★
            prompt = f"""
            あなたは伝説の相場師「バフェット達夫 (Ver.24.3 Fixed)」です。
            ユーザー: {user_name}, モード: {risk_mode}
            
            **【重要指示】**
            1. 数字はプレーンなテキストで出力せよ（例: $95000）。`$95,000`はOKだが、**「~」や「*」などのマークダウン装飾を数字の中に混ぜるな**。文字化けの原因になる。
