...
<style>
    ...
    .ai-box li, .ai-box p { font-size: 16px !important; }
</style>
""", unsafe_allow_html=True)
...
# ★最強データ取得関数 (Binance -> Bybit -> Fallback)★
def get_crypto_derivatives(symbol):
    ...
    try:
        ...
        fr_data = requests.get(fr_url, headers=headers, timeout=3).json()
        oi_data = requests.get(oi_url, headers=headers, timeout=3).json()
