...
def check_password():
    ...
    <style>
        ...
        h1 { color: #F0B90B !important; }
        input { background-color: #2a2e39 !important; color: white !important; }
        button { background: linear-gradient(90deg, #F0B90B, #D8A000) !important; color: black !important; font-weight: bold !important; }
    </style>
...
def save_log(symbol, price, sentiment, risk_mode, analysis_text):
    log_entry = {
        ...
        "symbol": symbol, "price": price, "sentiment": sentiment, "risk_mode": risk_mode, "full_text": analysis_text
    }
    st.session_state['history'].insert(0, log_entry)

def get_last_analysis(symbol):
    for log in st.session_state['history']:
        if log['symbol'] == symbol: return log
    return None
...
st.markdown("""
<style>
    /* 全称セレクタ(*)は使わず、主要なテキスト要素にのみフォントを適用 */
    body, p, h1, h2, h3, h4, h5, h6, span, div, label, button, input, textarea, li, td, th {
        font-family: 'Arial', sans-serif !important;
    }
    
    /* アイコンフォントのクラスを除外設定 (これで矢印等は直る) */
    .material-icons, [data-testid="stSidebarNav"] span {
    ...
    .stTextInput input, .stNumberInput input { 
        background-color: #2a2e39 !important; color: #ffffff !important; 
    ...
    .stButton>button { 
        ...
        color: #000 !important; font-weight: 800 !important; border: none; height: 3.5em; border-radius: 4px; 
    ...
    .ai-box h1 { color: #F0B90B !important; font-size: 28px !important; border-bottom: 2px solid #2a2e39; padding-bottom: 15px; margin-bottom: 20px; }
    .ai-box h2 { color: #0ECB81 !important; font-size: 22px !important; margin-top: 30px; margin-bottom: 15px; border-left: 4px solid #0ECB81; padding-left: 10px; }
    .ai-box h3 { color: #848e9c !important; font-size: 18px !important; margin-top: 20px; font-weight: bold; }
    .ai-box th { background-color: #2b313a; color: #F0B90B !important; padding: 12px; border: 1px solid #363c4e; text-align: left; font-size: 16px !important; }
    .ai-box td { border: 1px solid #363c4e; padding: 12px; color: #e1e1e1; font-size: 16px !important; }
    .ai-box strong { color: #F0B90B !important; font-weight: bold; font-size: 16px !important; }
...
st.sidebar.caption("Ver.31.0 Final Release")
st.sidebar.markdown("---")

user_name = st.sidebar.text_input("お名前", value="ユーザー")
api_key = st.sidebar.text_input("Google API Key", type="password", help="APIキーを入力")
...
# ★最強データ取得関数 (Binance/Bybit/CoinGecko 3段構え)★
def get_crypto_derivatives(symbol):
    ...
    try:
        ...
        fr = float(exchange.fapiPublicGetPremiumIndex({'symbol': ticker})['lastFundingRate'])
        # OIは認証なしだと厳しい場合があるがトライ
        oi_data = exchange.fapiPublicGetOpenInterest({'symbol': ticker})
    ...
    try:
        ...
        if resp['retCode'] == 0:
            ...
            return float(res['fundingRate']), float(res['openInterest'])
    except: pass

    # 3. CoinGecko (API) - 最終手段
    try:
        # 仮想通貨IDの特定
        cg_id = "bitcoin" if symbol.upper() == "BTC" else "ethereum" if symbol.upper() == "ETH" else "ripple" if symbol.upper() == "XRP" else None
        if cg_id:
            # 詳細データから無理やり引っ張る (無料枠の限界)
            url = f"https://api.coingecko.com/api/v3/coins/{cg_id}"
            data = requests.get(url, headers=headers, timeout=3).json()
            # データがない場合は市場平均的なダミーを返さず0を返し、AIに「取得不可」と判断させる
            return 0.0, 0.0 
    except: pass
...
if analyze:
    if not api_key: st.warning("⚠️ API Keyを入れてください")
        ...
        if data is None: st.error("データ取得エラー")
            ...
            fr_disp = f"{funding_rate*100:.4f}%" if funding_rate != 0 else "取得不可"
            oi_disp = f"${open_interest:,.0f}" if open_interest != 0 else "取得不可"
            
            fr_col = "#F6465D" if funding_rate > 0.0001 else "#0ECB81" if funding_rate < 0 else "#fff"
            with m3: st.markdown(f'<div class="metric-card"><div style="color:#848e9c;font-size:12px;">Funding Rate (FR)</div><div style="color:{fr_col};font-size:24px;font-weight:bold;">{fr_disp}</div></div>', unsafe_allow_html=True)
            with m4: st.markdown(f'<div class="metric-card"><div style="color:#848e9c;font-size:12px;">Open Interest (OI)</div><div style="color:#F0B90B;font-size:24px;font-weight:bold;">{oi_disp}</div></div>', unsafe_allow_html=True)
            ...
            # ★神プロンプト (完全体)★
            prompt = f"""
            あなたは伝説の相場師「バフェット達夫 (Ver.30.0 God Mode)」です。
            ユーザー: {user_name}, モード: {risk_mode}
            
            **【重要指示】**
            1. 数字はプレーンなテキストで出力せよ（例: $95000）。装飾記号は使うな。
            2. 文字サイズを変えるマークダウン（#や##）は、見出し以外で使うな。
            3. テクニカル指標の数値を必ずすべて評価に組み込め。
            ...
            【🐋 先物・需給データ】
            - 金利(FR): {fr_disp} (0.01%基準。プラスならロング過多)
            - 建玉(OI): {oi_disp} (トレンドの強さ)
            - 需給(MFI): {data['MFI']:.1f} (80以上過熱、20以下売られすぎ)
            ...
            - ボリンジャー: {bb_st} (Upper: ${data['BB_up']:.2f}, Lower: ${data['BB_low']:.2f})
            - 構造(Fib): {fib_st}
