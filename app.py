...
def check_password():
    ...
    <style>
        ...
        h1 { color: #F0B90B; font-family: sans-serif; letter-spacing: 2px; }
        .stTextInput input { 
...
def save_log(symbol, price, sentiment, strategy, analysis_text):
    """分析結果をセッションに保存"""
    log_entry = {
        "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M"),
        "symbol": symbol,
        "price": price,
        "sentiment": sentiment, # LONG/SHORT/WAIT
        "strategy": strategy,   # メインシナリオなど
        "full_text": analysis_text
    }
    st.session_state['history'].insert(0, log_entry) # 新しい順に追加
...
st.sidebar.caption("Ver.21.0 Self-Correction")
st.sidebar.markdown("---")
...
if analyze:
    if not api_key: st.warning("⚠️ API Keyを入れてください")
        ...
        if data is None: st.error("データ取得エラー")
            ...
            if last_log:
                last_log_text = f"""
                【前回の分析 ({last_log['timestamp']})】
                - 当時の価格: ${last_log['price']:,.2f}
                - 当時の判断: {last_log['sentiment']}
                - 当時の内容(抜粋): {last_log['full_text'][:200]}...
                ※ この過去データと比較し、シナリオがどう変化したか、前回の予測が正しかったか否かを検証し、今回の分析の精度を上げよ。
                """
            ...
            with m2: st.markdown(f'<div class="metric-card"><div style="color:#848e9c;font-size:12px;">Fear & Greed</div><div style="color:{fg_col};font-size:24px;font-weight:bold;">{fg_val}</div></div>', unsafe_allow_html=True)
            ...
            prompt = f"""
            あなたは伝説の相場師「バフェット達夫 (Ver.21.0 Self-Correction)」です。
